...
class EnvioPushNotifitionsATiempoViaje implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $request = $this->viaje->compraBoletos
            ...
            ->map(function($item, $key) use ($data, $_viaje) {
                ...
                $arr['sound'] = 'default';
                $arr['body']  = "Su vuelo {$_viaje->origen->nombre}-{$_viaje->destino->nombre} está a tiempo.";
                $arr['data'] = $data;
        ...
        curl_close($curl);
        Notificacion::create([
            'descripcion' => "Su vuelo {$_viaje->origen->nombre}-{$_viaje->destino->nombre} está a tiempo.",
